...
from desed_task.nnet.MobileNet_CRNN_low import CRNN_DWS
...
from local.sed_trainer_stage_two_low_test import SEDTask4
...
def single_run(
    ...
    ##### model definition  ############
    sed_student = CRNN_DWS(**config["net"])
    #sed_student = CRNN_DWS(**config["net"])
    pytorch_total_params = sum(p.numel() for p in sed_student.parameters())
    ...
    if test_state_dict is None:
        ...
        pseudo_weak_df = pd.read_csv(config["data"]["pseudo_unlabeled_tsv"], sep="\t")
        pseudo_weak_set = WeakSet(
        ...
        if strong_real:
            ...
            tot_train_data = [strong_full_set, weak_set, pseudo_weak_set]
        else:
            tot_train_data = [synth_set, weak_set, pseudo_weak_set]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--conf_file",
        default="./confs/sed_stage_two_low_test.yaml",
