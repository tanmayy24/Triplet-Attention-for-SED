...
def adaptive_median_filter(pred, median_filter):
    if(isinstance(median_filter, list)):
        for m in range(pred.shape[1]):
            pred[:,m] = scipy.ndimage.filters.median_filter(pred[:,m], median_filter[m])
    else:
        pred = scipy.ndimage.filters.median_filter(pred, (median_filter, 1))
    return pred

def batched_decode_preds(
    strong_preds, weak_preds, filenames, encoder, decode_weak, thresholds=[0.5], median_filter=7, pad_indx=None,
):
    ...
    for j in range(strong_preds.shape[0]):  # over batches
        for c_th in thresholds:
            ...
            if decode_weak < 2: # weak prediction masking
                pred = pred > c_th
            pred = adaptive_median_filter(pred, median_filter)
...
def strong_preds(
    strong_preds, weak_preds, decode_weak=1, thresholds=[0.5], median_filter=7, pad_indx=None,
):
    """ Decode a batch of predictions to dataframes. Each threshold gives a different dataframe and stored in a
    dictionary
    ...
    for j in range(strong_preds.shape[0]):  # over batches
        for c_th in thresholds:
            ...
            if decode_weak < 2: # weak prediction masking
                pred = pred > c_th
            pred = adaptive_median_filter(pred, median_filter)
            pred = pred.astype(int).transpose()  
            results_pred.append(pred)
    results_pred=np.asarray(results_pred, dtype=np.float32)
    results_pred = torch.from_numpy(results_pred)
    return results_pred
