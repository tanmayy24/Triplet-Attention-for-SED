...
class FDY_CRNN(nn.Module):
    def __init__(self,
                 ...
                 attention=True,
                 **kwargs):
        super(FDY_CRNN, self).__init__()
        self.n_input_ch = n_in_channel
        self.attention = attention
        self.n_class = nclass

        self.cnn = CNN(n_in_channel=n_in_channel, activation=activation, conv_dropout=dropout, **kwargs)
        self.rnn = BiGRU(n_in=self.cnn.nb_filters[-1], n_hidden=n_RNN_cell, dropout=dropout_recurrent, num_layers=rnn_layers)
    ...
    def forward(self, x, pad_mask=None, temp=1): #input size : [bs, freqs, frames]
        #cnn
        ...
        strong = temp_sigmoid(strong, temp)
        if self.attention:
