...
from .MobileNet_low_test import ResidualCNN

def temp_sigmoid(x, temp):
    exp_val = -x/temp
    return 1/(1 + torch.exp(exp_val))

class CRNN_DWS(nn.Module):
    def __init__(
        self,
        n_in_channel=1,
        nclass=10,
    ...
    def forward(self, x, pad_mask=None, embeddings=None, temp=1):
    ...
    def train(self, mode=True):
        ...
        """
        super(CRNN_DWS, self).train(mode)
        if self.freeze_bn:
